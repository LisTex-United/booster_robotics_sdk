...
class Controller:
    ...
    def start_custom_mode(self):
        ...
        head = [JOINT_PARAMETERS[name] for name in head_names]
...
def main():
    ...
    for i in range(8):
        goal = {
            "Head_Yaw": limits_movement["Head_Yaw"][i % 2],
            "Head_Pitch": limits_movement["Head_Pitch"][(i+1) % 2]
        }
        controller.head_control(goal)
    ...
    ret = controller.client.ChangeMode(RobotMode.kPrepare)   
    print(f"Switched to prepare mode: {ret}")
